...
namespace ALGeo_GH
{
    public class SignedDistanceField : GH_Component
    {
        public SignedDistanceField() : base("SignedDistanceField", "SDF", "To generate the signed distance field from an input geometry", "ALGeo", "VolumetircMesh") { }
            ...
            Parallel.For(0, vz, zz =>
            {
                ...
                for (int yy = 0; yy < vy; yy++)
                {
                    ...
                    for (int xx = 0; xx < vx; xx++)
                    {
                        ...
                        allCorner[id] = SignedDistance(mm, p, voxel, closed);
                    }
                }
            });
        ...
        private static double SignedDistance(Mesh m, Point3d p, double voxel, bool closed)
        {
            Point3d cp = m.ClosestPoint(p);
            double dist = p.DistanceTo(cp);

            if (!closed)
                return dist; // unsigned fallback

            bool inside = m.IsPointInside(p, voxel * 0.5, true);
        ...
        public override Guid ComponentGuid
        {
            get
            {
                return new Guid("{D1D016EB-83CB-4285-926D-1B6C28855863}");
            }
        }
    }
}
